...
from storage import G, query_similar_chunks


def find_similar_past_decisions(query_text: str, k: int = 5):
    embedding = embed_text(query_text)
    return query_similar_chunks(embedding, k=k)


def why_did_x_fail(node_label: str):
    """Find a node by label and walk its incoming causal edges (what caused it)."""
    matches = [n for n, d in G.nodes(data=True) if d.get("label") == node_label]
    if not matches:
        return None
    node_id = matches[0]
    causes = [
        {"cause_node_id": u, "cause_node": G.nodes[u], "edge": data}
        for u, v, data in G.in_edges(node_id, data=True)
        if data.get("edge_type") == "causal"
    ]
    return {"node_id": node_id, "node": G.nodes[node_id], "causes": causes}
...
def what_did_x_lead_to(node_label: str):
    ...
    node_id = matches[0]
    effects = [
        {"effect_node_id": v, "effect_node": G.nodes[v], "edge": data}
        for u, v, data in G.out_edges(node_id, data=True)
        if data.get("edge_type") == "causal"
    ]
    return {"node_id": node_id, "node": G.nodes[node_id], "effects": effects}
